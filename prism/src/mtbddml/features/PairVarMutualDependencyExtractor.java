...
public class PairVarMutualDependencyExtractor extends VisitorFeatureExtractor {
    ...
    public PairVarMutualDependencyExtractor(ModulesFile modulesFile, VarPair varPair) {
        super(modulesFile);
    ...
    public Object visit(Update e) throws PrismLangException {
        ...
        for(int i = 0; i < n; i++) {
            ...
            if (varNames.contains(firstName)) {
                firstVarSet.add(e.getVarIdent(i).getName());
            }

            if (varNames.contains(secondName)) {
                secondVarSet.add(e.getVarIdent(i).getName());
            }
        }
